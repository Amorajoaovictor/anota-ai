...
import { backoffMs, claimNext, completeJob, enqueue, failJob, releaseStaleJobs } from './queue'
...
describe('fila de jobs', () => {
  ...
  it('reagenda com backoff crescente enquanto houver tentativa', async () => {
    ...
    const job = await failJob(store, { id: 'job-1', attempts: 2, maxAttempts: 5 }, new Error('timeout'), { now })

    expect(job.status).toBe('PENDING')
    expect(job.lastError).toBe('timeout')
    expect(job.runAt.getTime()).toBe(now.getTime() + backoffMs(2))
